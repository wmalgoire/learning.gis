...
var LayerFilters = {
  ...
  init: function() {
    ...
    //         if (LayerFilters.shouldPause == true) {
    //           LayerFilters.addTracksUpToIndex(value);
    //         }
    //       },
    //       showButtons: false
    //     },
    ...
    //   LayerFilters.slider = slider;
    //   LayerFilters.slider.startup();
    //   LayerFilters.initSlider();
    //   LayerFilters.refreshStartButton();
    //   LayerFilters.initSearch();
    // });
  },
  ...
  initSearch: function() {
    ...
    function filterTeamCodes() {
      ...
      //mapClient.extension.onFilterClick();
    }
  },
  ...
  tracksAnimationStart: function() {
    ...
    if (self.shouldPause) {
      ...
      query.geometry = mapClient.extension.getVisibleExtent();
  ...
  addTracksUpToIndex: function(value) {
    ...
    if (self.tracksAnimationGraphicsLayer != null) {
      dijit.byId("playTracksProgressBar").update({
        progress: value
      });
  ...
  updateProgressbarWithGraphic: function(graphic) {
    ...
    dojo.query(".dijitProgressBarTile").style('background', '' + mapClient.utils.rgbToHex(colorRGBA[0], colorRGBA[1], colorRGBA[2]));
  },
  ...
  createFilterCheckboxHTML: function(id, attribute, fieldname) {
    ...
    html += "<input type='checkbox' class='list_" + fieldname + "' " + checkedStr + " id='" + id + "' onclick='mapClient.extension.onFilterClick();' />";
    html += "</td><td>";
